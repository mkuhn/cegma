...
# 	$Id: cegma.pl,v 1.9 2011/07/19 17:31:12 keith Exp $
...
if ( $cutoff_file eq "$CEGMA/data/profiles_cutoff.tbl" ) {
    ...
    my $command = "completeness $options local_self.hmm_select.aln $output.cegma.id $CEGMA/data/completeness_cutoff.tbl > $output.completeness_report";
    &print_command("$command");
    system("$BIN/$command") && die "Can't run $command\n";
}
...
# first check that FASTA headers don't include any examples with just digits before a space character
    ...
    system("$BIN/$genome_map_command") && &go_to_die("FATAL ERROR! $?: \"$!\"\n");
...
sub run_local_map {
    ...
    system("$BIN/$local_map_command") && &go_to_die("FATAL ERROR! $?: \"$!\"");

    if ( -z "local.genewise.gff" ) {
        &go_to_die("No orthologous genes found in the first round.\n\n   genewise didn't give any suitable aligment.");
    }
