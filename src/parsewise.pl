...
while (<GENEWISE>){
	...
	if (exists($output_line[2]) 
			&& $output_line[2] eq "match"){
		$sequence_id = $output_line[0];	
	}

	if (exists($output_line[3]) 
			&& $output_line[3] eq "LOOP_STATE"){
	...
	if (exists($output_line[1]) 
			&& $output_line[1] eq "output"){
		...
		if ($sequence_id && $entry){
			...
			# in very exceptional cases genewise do not include a loop state at the begining
